...
של 10 ש"ח לכל סט בנפרד, מטרה: תנועה (קליקים על לינק) לפרופיל האינסטגרם `ben_nahum_1`.

**הערה על ה-objective**: הניסיון הראשוני היה להשתמש ב"ביקורים בפרופיל אינסטגרם"
הילידי של Meta (`OUTCOME_ENGAGEMENT` + `destination_type=INSTAGRAM_PROFILE` +
`optimization_goal=PROFILE_AND_PAGE_ENGAGEMENT`) - זה עבד כשנבנה ידנית ב-Ads Manager,
אבל נכשל עקבית דרך ה-API הציבורי (גם עם קמפיין חדש, גם עם טוקן משתמש אישי, גם עם
גרסת API עדכנית) - כנראה בר-יצירה רק דרך זרימת ה-guided creation הפנימית של Ads
Manager. הוחלף ב-`OUTCOME_TRAFFIC` + `optimization_goal=LINK_CLICKS` (מוביל ללינק
חיצוני - כתובת הפרופיל) - אותו pattern בדיוק כמו קמפיין הדיסקורד הפעיל בחשבון הזה.
...
- ודאו ש-13 הסרטונים תואמים בפועל לקבצים בתיקיית הדרייב (השדה `match` בכל פריט ב-
  ...
  במקום לנחש.
- **טקסט המודעה נלקח מקובץ בתוך תת-התיקייה בדרייב, לא מ-`config.py`**: כל תת-תיקייה
  מכילה גם את הווידאו וגם קובץ טקסט (Google Doc או `.txt`) - זה הטקסט הראשי שישמש
  בפועל בקריאטיב. השדה `message` ב-`config.VIDEOS` הוא רק גיבוי, למקרה שלא נמצא קובץ
  טקסט בתיקייה. מקפים ארוכים/מיוחדים בטקסט (כמו `—` או `–`, נפוצים בטקסט שנכתב ב-
  Google Docs) מוחלפים אוטומטית במקף קצר רגיל (`-`).
- **`CAMPAIGN_OBJECTIVE` / `OPTIMIZATION_GOAL`** - אומתו בפועל מול ה-API (ראו הערה
  למעלה) ומבוססים על pattern שכבר עובד בחשבון הזה. אם בכל זאת `create_ad_set` נכשל
  בשגיאה על `optimization_goal` לא תקין, **הודעת השגיאה מ-Meta עצמה תפרט את כל הערכים
  התקינים הקיימים כרגע** (או שאפשר להשתמש ב-`debug_list_adsets.py` כדי להשוות מול
  ad set קיים שמצליח) - עדכנו
