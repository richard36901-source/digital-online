...
## יש לכם כבר חשבון TikTok Business (מפרסם)?

מצוין - זה חוסך שלב. אין צורך ביצירת חשבון TikTok חדש: משתמשים באותו חשבון קיים גם
לרישום האפליקציה ב-developers.tiktok.com וגם ל-`python main.py authorize`. חשבון
Business נותן טווח מלא של אפשרויות `privacy_level` (כולל `PUBLIC_TO_EVERYONE`) לעומת
חשבון אישי רגיל שבו האפשרויות מוגבלות יותר, וגם ה-audit נוטה לעבור מהר יותר לחשבון
עם פעילות פרסום קיימת.
